...
async def get_user_by_id(user_id: str, session: AsyncSession) -> UserPublic:
    statement = select(User).where(and_(User.id == user_id), User.is_deleted == False)
...
async def update_user(user_id: UUID, user_to_update: UserSave, session: AsyncSession) -> UserPublic:
    try:
